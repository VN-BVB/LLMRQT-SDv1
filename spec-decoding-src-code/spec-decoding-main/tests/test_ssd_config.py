...
"""CPU-only tests for core.ssd Phase 0 (config + types, no GPU/HF)."""

from __future__ import annotations

import unittest

from spec_decoding.ssd import (
    CacheKey,
    Eagle3Config,
    SpeculateResult,
    VerifyResult,
...
class TestEagle3Imports(unittest.TestCase):
    def test_lazy_generator_import(self):
        from spec_decoding.ssd import SSDGenerator

        self.assertTrue(SSDGenerator.__name__ == "SSDGenerator")

    def test_generator_requires_fc(self):
        from spec_decoding.ssd import SSDGenerator

        with self.assertRaises(TypeError):
            SSDGenerator(target=object(), draft=object(), cfg=Eagle3Config())
