...
"""Tests for spec_decoding.ssd_parallel.runner (CPU, no NCCL)."""
...
from spec_decoding.ssd_parallel import Eagle3Config, SSDGenerator, build_ssd_fc
...
class TestSSDParallelRunner(unittest.TestCase):
    def test_sync_spec_cache_ok(self):
        ...
        self.assertIsNone(gen.draft_client)

    def test_draft_async_requires_path(self):
        target = _Target(hidden=8, n_layers=4)
        draft = _CfgModel(hidden=8)
        cfg = Eagle3Config(
            draft_async=True,
