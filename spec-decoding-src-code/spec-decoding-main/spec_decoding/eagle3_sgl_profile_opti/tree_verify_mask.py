...
"""
EAGLE2 要素之三：专用 Verify Attention（树形掩码，不是普通 causal）

一次 target forward 里若要把 多个树节点 当作序列上的多个位置一起算
self-attention，不能用标准下三角 mask：那样会允许 兄弟子树 互相 attend，
未选中的 draft 分支会污染当前位置的表示，verify 结果 不再等价于只沿
选中路径逐步算大模型。

EAGLE2 的做法：位置 i 的 Q 只能 attend 树意义上从根到 i 路径上的 key
...
def layout_tree_position_ids(layout: TreeVerifyLayout, past_len: int) -> torch.LongTensor:
    """Absolute position_ids for batched tree verify, shape [L]."""
...
def build_tree_allow_mask_4d(
    ...
    *,
    ...
    A = _ancestor_self_matrix(layout.parent, n, device)
...
def _ancestor_self_matrix(
    parent: Sequence[int], n: int, device: torch.device
) -> torch.Tensor:
    """[n, n] bool：A[i, k]=True 当且仅当 k 是 i 的祖先或 k==i（= k 在 root→i 路径上）。

    向量化构造：从每个节点出发沿 parent 同步上爬，每层一次散射写入；循环次数 ≤ 树深
    （提前 break），替代原 [n×(past+n)] 双重 Python 循环 + 逐元素标量写。
    """
    A = torch.eye(n, dtype=torch.bool, device=device)
    if n == 0:
        return A
    parent_t = torch.tensor(list(parent), dtype=torch.long, device=device)
    idx = torch.arange(n, device=device)
    cur = parent_t.clone()
    for _ in range(n):  # 上限 n，按树深提前 break
        valid = cur >= 0
        if not bool(valid.any()):
            break
        A[idx[valid], cur[valid]] = True
        cur = torch.where(valid, parent_t[cur.clamp(min=0)], cur)
    return A
...
def build_tree_cross_attn_bias_with_prefix(
    ...
    *,
    ...
    neg = torch.finfo(dtype).min
    A = _ancestor_self_matrix(layout.parent, n, device)
    allow = torch.zeros((n, past_seq_len + n), dtype=torch.bool, device=device)
    allow[:, :past_seq_len] = True
    allow[:, past_seq_len:] = A
