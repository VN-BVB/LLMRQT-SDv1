...
"""
真·EAGLE3-SGL 端到端：真实 target + 真实 EAGLE3 draft 权重，并把各项性能数据写入 perf.log。

模型来源（与 sglang / ssd 的 EAGLE3 配置一致）：
...
def _load_target(target_id: str, device, dtype):
    ...
    for mid in [target_id, fallback]:
        try:
            print(f"[eagle3sgl-e2e] loading target {mid} ...", flush=True)
            tok = AutoTokenizer.from_pretrained(mid)
            model = AutoModelForCausalLM.from_pretrained(mid, torch_dtype=dtype).to(device).eval()
            print(f"[eagle3sgl-e2e] target ready: {mid}", flush=True)
            return tok, model, mid
        except Exception as e:  # gated / network
            print(f"[eagle3sgl-e2e] failed to load {mid}: {e}", flush=True)
            last_err = e
    raise RuntimeError(f"could not load target; last error: {last_err}")


def main() -> None:
    p = argparse.ArgumentParser(description="Real EAGLE3-SGL on Llama-3.1-8B")
    ...
    p.add_argument("--cuda-graph", action="store_true",
                   help="Step5/6：static KV + verify CUDA graph；默认同时取消 target replay")
    p.add_argument("--keep-replay", action="store_true",
                   help="保留 Step5 target replay，用于与 Step6 做 A/B")
    ...
    cfg = Eagle3SglConfig(
        ...
        eliminate_replay=not args.keep_replay,
    )
    eagle_layers = cfg.resolve_eagle_layers(n_layers)  # 默认 [2, L//2, L-3]
    print(f"[eagle3sgl-e2e] eagle_layers={eagle_layers} draft_vocab_size={draft_vocab_size}", flush=True)
    ...
    # 自回归 draft（KV + RoPE + feature 递归，对齐 SGLang EAGLE3）
    ...
    # 预热（含 CUDA graph 捕获 / static cache 分配 / kernel autotune），不计入计时。
    # 必须用与计时相同的 max_new_tokens，否则 static cache 容量(need)不同 → 计时段内重分配+重捕获。
    with torch.inference_mode():
        gen.generate(input_ids, max_new_tokens=args.max_new_tokens, eos_token_id=eos)
    gen.n_rounds = 0
    gen.n_accepted_tokens = 0
    gen.n_matched_draft_tokens = 0
    gen.n_proposed_draft_tokens = 0
    ...
    # 用 baseline 贪心（逐 token）作为基准时间参照（先预热一次，避免冷启动/cache 分配计入）
    with torch.inference_mode():
        target.generate(input_ids, max_new_tokens=args.max_new_tokens, do_sample=False,
    ...
    with open(args.perf_log, "a") as f:
        ...
        f.write(rec)
        f.write("\n")
    print(f"[eagle3sgl-e2e] perf appended to {os.path.abspath(args.perf_log)}")
    assert lossless, "EAGLE3 output length or token ids diverged from clean greedy"
    print("[eagle3sgl-e2e] OK (strict length + token-id match)")
