...
"""分阶段真实墙钟（CUDA event 同步计时）—— 优化版 eagle3_sgl_profile_opti。

阶段：target_prefill / draft_prefill_init / draft_build_tree / target_verify /
      target_replay / draft_extend。用于刷新 %wall + ms/轮 表。
"""
...
def main():
    ...
    draft = build_eagle3_sgl_draft(target, el, num_layers=1,
    ...
    if args.cuda_graph:
        # graph 路径下 verify 走 StaticGraphVerifier.verify（graph.replay），不调用 target_m.forward；
        # 类级 patch 让复用的 verifier 实例也计时（warmup 已完成捕获，patch 在其后，安全）。
    ...
    print("\n==================== opti 分阶段墙钟（CUDA event 同步计时）====================")
    print(f"new_tokens={n_new} rounds={gen.n_rounds} accept/round={accept:.2f} "
          f"wall={wall:.2f}s spec_tok/s={n_new / wall:.2f} ms/round={1e3 * wall / rounds:.1f}")
    print(f"{'stage':<24}{'calls':>8}{'total_ms':>12}{'%wall':>9}{'ms/round':>11}{'ms/call':>10}")
    for k in order:
        ms = T.total[k] * 1e3
        c = T.calls[k]
        print(f"{k:<24}{c:>8}{ms:>12.1f}{100 * T.total[k] / wall:>8.1f}%{ms / rounds:>11.2f}{ms / max(c, 1):>10.2f}")
    print(f"{'(measured sum)':<24}{'':>8}{measured * 1e3:>12.1f}{100 * measured / wall:>8.1f}%{measured * 1e3 / rounds:>11.2f}")
    print(f"{'(unmeasured/python)':<24}{'':>8}{(wall - measured) * 1e3:>12.1f}{100 * (wall - measured) / wall:>8.1f}%{(wall - measured) * 1e3 / rounds:>11.2f}")
