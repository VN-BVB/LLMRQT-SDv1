...
"""
EAGLE 轻量 draft 网络（用于 eagle2 树形投机）：

  - 复用 target 的 embedding 与 lm_head（不另建词表/输出头）；
  - fc = Linear(2H, H)：把 token_embedding 与 target hidden 拼接后融合；
  - 通常 1 层 decoder（可配 num_layers）；

与 eagle 的唯一区别：eagle2 的树展开用 累计 log 概率 做 beam/剪枝，所以这里的
make_eagle2_draft_topk_fn 返回的是 log-softmax 后的 top-k 分数（而非原始 logits），
便于 tree_draft 直接当 log 概率累加。权重需由 EAGLE 训练得到，若随机初始化接受率
低，但 target verify 也能保证语义正确性。
"""
...
def _sdpa_gqa(
    ...
    真实 EAGLE draft（如 LLaMA3-8B / Qwen2.5-32B）是 GQA，不支持gqa会导致权重 shape 不匹配。
    """
...
class _DraftDecoderLayer(nn.Module):
    """Llama style单层layer：(可选 input_norm) → self-attn → 残差 → post_norm → SwiGLU → 残差。"""
    ...
    def __init__(
        ...
        **fk,
        ...
        """构建单层layer权重：attn 的 q/k/v/o 投影（GQA）+ SwiGLU MLP（gate/up/down）+ RMSNorm。

        qkv_bias：q/k/v_proj 是否带 bias（Qwen2/2.5 为 True；Llama 为 False）。o_proj 始终无 bias。
        """
        super().__init__()
    ...
    def _attn(self, x: torch.Tensor) -> torch.Tensor:
        """（GQA）多头因果自注意力。draft 通常只喂 1 个 token（S=1），此处仍按通用因果实现。"""
        b, s, h = x.shape
        q = self.q_proj(x).view(b, s, self.n_heads, self.head_dim).transpose(1, 2)
        k = self.k_proj(x).view(b, s, self.n_kv_heads, self.head_dim).transpose(1, 2)
        v = self.v_proj(x).view(b, s, self.n_kv_heads, self.head_dim).transpose(1, 2)
        # 因果 SDPA；无 rotary（decode阶段单 token 位置无影响）
...
class Eagle2DraftModel(nn.Module):
    """
    轻量 EAGLE draft：复用 target embedding/lm_head + fc(2H→H) + num_layers 层 decoder。
    ...
    def __init__(
        ...
        *,
        ...
        use_final_norm=False：lm_head 前不加最终 RMSNorm（视你用的模型而定）。默认 True（随机初始化的轻量版保留）。
        """
        super().__init__()
        tm = _unwrap(target)
        cfg = tm.config
        ...
        # 自动判定 q/k/v 是否带 bias（draft 沿用 target 的注意力约定）：Qwen2/2.5 有、Llama 无。
        qkv_bias = False
        try:
        ...
        # 新建参数默认跟随 target 的 device/dtype，保证 forward 中三者一致。
        if device is None:
    ...
    def forward(
        ...
        for layer in self.layers:  # 通常 1 层，多层接受率会好点，但性能差点
            x = layer(x)
        x = self.norm(x)
...
def build_eagle2_draft(
    ...
    *,
    ...
    """构造复用 target embedding/lm_head 的轻量 EAGLE draft（用于 eagle2 树形投机）。

    - draft_weights_path=None（默认）：随机初始化（可跑通、接受率低，正确性由 target verify 保证）。
    - draft_weights_path=<dir 或 .bin/.safetensors/.pt>：加载 官方 EAGLE draft 权重
      （fc / layers.0.* / 复用 target embed），获得真实接受率。
      EAGLE2 与 EAGLE1 共用同一份 draft checkpoint（EAGLE2 只改推理期树展开策略）。
    """
    ...
    if draft_weights_path is not None:
        load_eagle2_draft_weights(m, draft_weights_path)
    return m
...
def _translate_eagle_name(name: str) -> str:
    """官方 EAGLE 参数名 → 本模块命名：去 model. 前缀、self_attn./mlp. 中间名。
...
def load_eagle2_draft_weights(
    draft_model: Eagle2DraftModel,
    path: str,
    *,
    strict: bool = False,
) -> dict:
    """把官方 EAGLE draft 权重加载进 draft_model（与 EAGLE1 checkpoint 兼容）。

    处理：名称翻译（去 self_attn./mlp.）、跳过 embed_tokens（复用 target）。
    ...
    for k, v in src.items():
        ...
        local[_translate_eagle_name(k)] = v
...
def make_eagle2_draft_topk_fn(draft_model: Eagle2DraftModel, topk: int):
    """工厂：把 Eagle2DraftModel 包成 tree_draft.DraftTopkFn。

    返回 (topk_ids, topk_log_probs)：分数是 log_softmax 后的值，便于 eagle2 树展开里
    把每步分数当 log 概率累加做 beam / 剪枝。
    """
