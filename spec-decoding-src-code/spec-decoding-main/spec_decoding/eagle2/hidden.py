...
"""
EAGLE2 要素之一：在 Target hidden 上建 Draft（条件信号从哪来）

EAGLE2 相对只喂 token 的 draft：小模型每一步还应看见 大模型对当前前缀
...
本文件职责单一：从 HF CausalLM 前向中取出该 h（output_hidden_states）。
把 h 接到你的 draft 结构里调用。
"""
...
# -----------------------------------------------------------------------------
# 完整 EAGLE2 draft 侧：每个当前前缀对应一个 last-token hidden，供 topk 树
# 上每个节点扩展时作为条件。
# -----------------------------------------------------------------------------
