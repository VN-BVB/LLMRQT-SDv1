...
- verify_attn_backend=triton_tree（topk>1 extend_attention_fwd + custom_mask。
"""
...
def select_append_from_full_tree_logits(
    ...
    """
    Pick best leaf by greedy match length (same tie-break as runner.generate_step_payload).
...
# 在 cache 上临时 extend 后 crop 还原（不污染 past）。root 接受用 recovery_logits
# （out_ids 之后那一位 logits）。
# -----------------------------------------------------------------------------
def _extend_then_crop(target_m, tree_tokens, past_key_values, *, position_ids, attention_mask):
    ...
    比深拷贝整份 cache 便宜得多（只多存/截掉 L 个位置的 KV）；。
    """
...
def _select_append_root_child(
    ...
    """从每节点输出预测其子节点的 node_logits + root 用 recovery_logits 做 greedy 接受。"""
    best_m = -1
    best_li = 10**9
    best_next = 0
    best_leaf = 0
    for li, leaf in enumerate(leaves):
        ...
        for k in range(len(idxs)):
            # parent→child 接受：第 k 个 draft token 应由其父节点的 logits 预测
...
def full_tree_verify_extend(
    ...
    *,
    ...
    ----------
    target_m
        完整 target *ForCausalLM（带 lm_head），用于一次 forward 得到 L 个树节点的 logits。
    layout
        draft 树的 verify 布局（TreeVerifyLayout：摊平后的 token_ids、parent 等），
        决定树内 attention 掩码与 position_ids。
    past_key_values
        runner 持有的、对应已接受前缀 out_ids 的 KV cache。本函数在其上临时 extend
        L 个树节点，forward 后 crop 还原，不污染 runner 的 past。
    past_len
        已接受前缀长度 P = len(out_ids)，用于构造 [L, P+L] cross 树掩码（树节点可 attend
        前缀 KV，树内按 parent 链约束）。
    recovery_logits
        target 在前缀末位置预测下一 token的 logits [V]。用于 root 层接受：draft 树
        第一个节点与 recovery_logits 的 argmax 比对（与干净贪心对齐）。
    parent, token_ids
        draft 树摊平数组（与 TreeDraftResult 同构）：token_ids[i] 为节点 token，
        parent[i] 为父下标（根为 -1）。供 _select_append_root_child 沿叶路径做 greedy 接受。
    leaves
        所有叶子节点下标列表。对每条根→叶路径比对 target logits，选接受长度最长的一条。
    device
        计算设备。

    Returns
    -------
    append_list, best_match_len, best_leaf_index, best_path_tokens

    append_list 为本轮要追加的 token 序列；在 past 上临时 extend 后 crop 还原
    （不污染 runner 的 past）；。
    """
    L = len(layout)
