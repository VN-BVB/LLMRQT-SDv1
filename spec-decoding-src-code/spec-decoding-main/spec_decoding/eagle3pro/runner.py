...
class Eagle3SglGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        self.n_tree_causal_corrections = 0
        # 并行 tree 结论与 canonical 单-token commit 首次分歧的轮数。
        self._last_matched_draft_tokens = 0
        self._last_proposed_draft_tokens = 0
        # CUDA Graph 需要跨 generate() 保持同一批 KV/input 地址；因此 workspace
        # 和 graph runner 都由 generator 持有，而不是每条 prompt 临时创建。
        self._graph_target_cache = None
        self._graph_draft_cache = None
        self._cuda_graph_runner = None
        self._active_cuda_graph = None
        self._cuda_graph_unavailable_reason: Optional[str] = None
        self._cuda_graph_warning_emitted = False

    @property
    def cuda_graph_stats(self) -> dict[str, Any]:
        """Return capture/replay counters for benchmark reports."""
        if self._cuda_graph_runner is not None:
            stats = dict(self._cuda_graph_runner.stats)
        else:
            stats = {
                "enabled": False,
                "disabled_reason": self._cuda_graph_unavailable_reason,
                "target_captures": 0,
                "draft_captures": 0,
                "target_replays": 0,
                "draft_replays": 0,
        ...
        stats["requested"] = bool(self.cfg.cuda_graph)
        return stats

    def _disable_cuda_graph(self, reason: str) -> None:
        self._cuda_graph_unavailable_reason = reason
        self._active_cuda_graph = None
        if self._cuda_graph_runner is not None:
            self._cuda_graph_runner.disable(reason)
        if not self._cuda_graph_warning_emitted:
            warnings.warn(
                f"EAGLE3Pro CUDA Graph disabled; falling back to eager: {reason}",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    def _build_tree(
        ...
        if self.autoregressive_draft:
            ...
            if draft_state is not None:
                ...
                if getattr(root_kv, "_eagle3pro_draft_flash_cache", False):
                    root_kv = root_kv.as_tuple()
                    # 多步候选链在临时 tuple 上 fork；不会把未验证 token 写进主 Flash KV。
            else:
                if acts_all is None:
    ...
    def _verify_chain_and_commit(
        ...
        past_before: Any,
        draft_chain: Union[TreeDraftResult, torch.LongTensor],
        *,
        ...
        if isinstance(draft_chain, TreeDraftResult):
            token_ids = draft_chain.token_ids
            expected_parent = [-1] + list(range(max(0, len(token_ids) - 1)))
            if draft_chain.parent != expected_parent:
                raise ValueError("fast chain verify requires a single top-1 chain")
            candidate_ids = torch.tensor(
                [token_ids], device=device, dtype=torch.long
            )
        else:
            candidate_ids = draft_chain
            if (
                candidate_ids.ndim != 2
                or candidate_ids.shape[0] != 1
                or candidate_ids.shape[1] < 1
                or candidate_ids.device != device
                or candidate_ids.dtype != torch.long
            ):
                raise ValueError(
                    "device-resident draft chain must be [1, steps] torch.long"
                )
        ...
        if getattr(past_before, "_eagle3pro_flash_cache", False):
            full_attn = {"full_attention": None}
            # EAGLE3PRO: custom Flash KV attention 自己施加右下对齐 causal mask，跳过 HF 4D mask 构造。
        else:
            full_attn = torch.ones(
                (1, before + q_len), device=device, dtype=torch.long
            )
        graph_runner = self._active_cuda_graph
        if graph_runner is not None and q_len == graph_runner.target_query_len:
            try:
                pred_ids, all_shift_acts = graph_runner.verify_target(
                    verify_ids, before=before
                )
            except Exception as exc:
                from .cuda_graph import Eagle3ProCudaGraphUnavailable

                if not isinstance(exc, Eagle3ProCudaGraphUnavailable):
                    raise
                self._disable_cuda_graph(str(exc))
                graph_runner = None
        if graph_runner is None:
            ...
            pred_ids = lm_head(hidden_states).argmax(dim=-1)
        if (
            not self.cfg.gpu_resident_top1
            and token_ids is not None
            and len(token_ids) == 1
        ):
            # Exact A/B fallback for the pre-port path: candidate construction
            # and accepted output both make the original CUDA->Python->CUDA
            # round trip.  This branch is intentionally benchmark-only.
            pred_list = pred_ids.tolist()
            matched = int(pred_list[0] == token_ids[0])
            append_list = token_ids[:matched] + [pred_list[matched]]
            ...
            append = torch.tensor(
                [append_list], device=device, dtype=torch.long
            )
            return append, shift_acts, kept_draft

        # The proposal and predictions remain on device.  Only the prefix-match
        # length crosses to Python, once, because cache crop/statistics need an
        # integer.  This removes draft topk().tolist() + token H2D on the
        # num_steps=1 hot path while preserving the exact greedy rule.
        if candidate_ids.shape[1] == 1:
            matched = int(pred_ids[0].eq(candidate_ids[0, 0]).item())
        else:
            prefix_matches = (
                pred_ids[:-1].eq(candidate_ids[0]).to(torch.int32).cumprod(0)
            )
            matched = int(prefix_matches.sum().item())
        next_token = pred_ids[matched].view(1, 1)
        append = torch.cat([candidate_ids[:, :matched], next_token], dim=1)
        append = append[:, :max_append_tokens]
        ...
        kept_draft = min(matched, int(append.shape[1]))
        past_before.crop(before + 1 + kept_draft)
        # 保留 pending 与已接受 draft 的 KV；末尾 target token 留到下一轮一起验证。
        shift_acts = all_shift_acts[:, : int(append.shape[1]), :]
    ...
    def generate(
        ...
        *,
        ...
        output_length = int(out_ids.shape[1])
        if fast_chain:
            output_buffer = torch.empty(
                (1, output_length + int(max_new_tokens)),
                device=device,
                dtype=out_ids.dtype,
            )
            output_buffer[:, :output_length].copy_(out_ids)
            out_ids = output_buffer[:, :output_length]
        graph_eligible = (
            self.cfg.cuda_graph
            and fast_chain
            and self.cfg.gpu_resident_top1
            and torch.cuda.is_available()
            and input_ids.is_cuda
            and getattr(self.target_m, "_eagle3pro_flash_kv_enabled", False)
            and getattr(self.draft_model, "_eagle3pro_flash_kv_enabled", False)
        )
        self._active_cuda_graph = None
        if self.cfg.cuda_graph and not graph_eligible:
            self._disable_cuda_graph(
                "requires CUDA, batch-1 topk=1 GPU-resident path, "
                "and packed-vllm-flashkv target/draft caches"
            )
        ...
        if fast_chain and getattr(
            self.target_m, "_eagle3pro_flash_kv_enabled", False
        ):
            ...
            cache_capacity = (
                max(requested_capacity, int(self.cfg.cuda_graph_max_seq_len))
                if graph_eligible
            ...
            past = Eagle3FlashKVCache.from_dynamic(
                ...
                reuse=self._graph_target_cache if graph_eligible else None,
                graph_safe=graph_eligible,
            )
            if graph_eligible:
                self._graph_target_cache = past
            # EAGLE3PRO: top-1 prefill 后只转换一次，decode 阶段复用预分配 KV。
        ...
        if self.autoregressive_draft:
            ...
            if (
                fast_chain
                and
                getattr(self.draft_model, "_eagle3pro_flash_kv_enabled", False)
            ):
                from .target_optim import Eagle3DraftFlashKVCache

                requested_capacity = (
                    int(out_ids.shape[1])
                    - 1
                    + int(max_new_tokens)
                    + max(1, min(int(self.cfg.num_steps), int(self.cfg.max_tree_nodes)))
                )
                # 多 step proposal 会先把未验证链暂写到 accepted prefix 尾部；
                # 即使最终只保留一个 token，也必须为最后一轮 lookahead 预留空间。
                cache_capacity = (
                    max(requested_capacity, int(self.cfg.cuda_graph_max_seq_len))
                    if graph_eligible
                    else requested_capacity
                )
                d_kv = Eagle3DraftFlashKVCache.from_tuple(
                    d_kv,
                    max_cache_len=cache_capacity,
                    inv_freq=self.draft_model.midlayer.inv_freq,
                    reuse=self._graph_draft_cache if graph_eligible else None,
                    graph_safe=graph_eligible,
                )
                if graph_eligible:
                    self._graph_draft_cache = d_kv
                # 主状态原地更新；多步建链通过 as_tuple() 只读视图在临时 KV 上 fork。
        ...
        if graph_eligible:
            from .cuda_graph import Eagle3ProCudaGraphRunner

            graph_chain_steps = min(
                int(self.cfg.num_steps), int(self.cfg.max_tree_nodes)
            )
            if (
                self._cuda_graph_runner is None
                or not self._cuda_graph_runner.compatible(past, draft_state.kv)
            ):
                self._cuda_graph_runner = Eagle3ProCudaGraphRunner(
                    self.target_m,
                    self.draft_model,
                    self.eagle_layers,
                    past,
                    draft_state.kv,
                    target_query_len=graph_chain_steps + 1,
                    max_draft_append=graph_chain_steps + 1,
                    warmup_iters=self.cfg.cuda_graph_warmup_iters,
                )
            if self._cuda_graph_runner.enabled:
                self._active_cuda_graph = self._cuda_graph_runner
        ...
        while new_tok < max_new_tokens:
            ...
            # ④ 单轮：建树 → verify → append
            if fast_chain:
                if self.cfg.gpu_resident_top1:
                    # 固定 top-1 链不需要 Python parent/depth 元数据。即使 num_steps>1，
                    # proposal 也全程留在 GPU，并在主 Flash KV 的未使用尾部暂存；
                    # 建链后回滚逻辑长度，commit 再覆盖尾部的已接受部分。
                    graph_runner = self._active_cuda_graph
                    first_id = self.draft_model.greedy_target_ids(
                        draft_state.root_logits
                    )
                    if graph_runner is not None and self.cfg.num_steps > 1:
                        try:
                            draft_chain = graph_runner.build_draft_chain(
                                first_id[:, :1],
                                draft_state.root_hidden,
                                start_position=draft_state.prefix_len - 1,
                            )
                        except Exception as exc:
                            from .cuda_graph import Eagle3ProCudaGraphUnavailable
                            ...
                            self._disable_cuda_graph(str(exc))
                            graph_runner = None
                    if graph_runner is None or self.cfg.num_steps == 1:
                        draft_chain = expand_draft_chain_ar_gpu(
                            self.cfg,
                            self.draft_model,
                            draft_state.root_logits,
                            draft_state.root_hidden,
                            draft_state.kv,
                            int(out_ids.shape[1]) - 1,
                        )
                    proposed_draft = int(draft_chain.shape[1])
                else:
                    draft_chain = self._build_tree(
                        out_ids, last_t, root_acts, acts_all, draft_state
                    )
                    proposed_draft = len(draft_chain.token_ids)
                ...
                # 多分支树的并行 attention 与标准单-token decode 可能在近似并列
                # logits 上产生不同 argmax。commit 按 target 的 canonical causal
                # 路径逐 token 核对；首个分歧立即写入 target 修正 token 并停止本轮。
                candidate_append = append
                old_root_acts = root_acts
                committed_tokens: List[torch.Tensor] = []
                committed_acts: List[torch.Tensor] = []
                for append_idx in range(int(candidate_append.shape[1])):
                    proposed_token = candidate_append[:, append_idx : append_idx + 1]
                    target_token = recovery_logits.argmax(dim=-1).view(1, 1)
                    disagrees = bool(target_token.ne(proposed_token).item())
                    commit_token = target_token if disagrees else proposed_token
                    if disagrees:
                        matched_draft = min(matched_draft, append_idx)
                        self.n_tree_causal_corrections += 1

                    full_attn = torch.ones(
                        (1, int(past.get_seq_length()) + 1),
                        device=device,
                        dtype=torch.long,
                    )
                    out_sync, token_acts = forward_target_selected_eagle_acts(
                        self.target_m,
                        self.eagle_layers,
                        last_token_only=False,
                        input_ids=commit_token,
                    ...
                    committed_tokens.append(commit_token)
                    committed_acts.append(token_acts)
                    if disagrees or (
                        eos_token_id is not None
                        and int(commit_token.item()) == eos_token_id
                    ):
                        break

                append = torch.cat(committed_tokens, dim=1)
                append_acts = torch.cat(committed_acts, dim=1)
            ...
            if self.autoregressive_draft and draft_state is not None:
                ...
                if graph_runner is not None:
                    try:
                        d_logits, d_hidden, d_kv = graph_runner.extend_draft(
                            append,
                            draft_shift_acts,
                            start_position=p_old - 1,
                        )
                    except Exception as exc:
                        from .cuda_graph import Eagle3ProCudaGraphUnavailable
                        ...
                        graph_runner = None
                if graph_runner is None:
                    if getattr(draft_state.kv, "graph_safe", False):
                        draft_state.kv.set_length(p_old - 1)
                        # Graph 中途失效时，先把 device cache_seqlens 恢复到 Python 逻辑位置。
            ...
            if output_buffer is not None:
                append_size = int(append.shape[1])
                output_buffer[
                    :, output_length : output_length + append_size
                ].copy_(append)
                output_length += append_size
                out_ids = output_buffer[:, :output_length]
            else:
