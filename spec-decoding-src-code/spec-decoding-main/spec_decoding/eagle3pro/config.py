...
class Eagle3SglConfig:
    ...
    gpu_resident_top1: bool = True
    """For the one-step top-1 fast path, keep the draft candidate on CUDA.

    Disable only for A/B profiling against the legacy Python TreeDraftResult
    materialization and GPU-to-CPU-to-GPU token round trip.
    """

    cuda_graph: bool = False
    """Capture the batch-1, top-1 decode hot path as reusable CUDA Graphs.

    For ``num_steps=S``, the implementation records one target verify graph with
    shape ``[1, S+1]`` and draft commit graphs ``[1, 1]`` through ``[1, S+1]``.
    Unsupported configurations fail closed to the existing eager path.
    """

    cuda_graph_max_seq_len: int = 1024
    """Persistent target/draft KV workspace capacity used by CUDA Graph replay."""

    cuda_graph_warmup_iters: int = 3
    """Side-stream eager warmups before each graph capture."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.cuda_graph_max_seq_len < 1:
            raise ValueError("cuda_graph_max_seq_len must be >= 1")
        if self.cuda_graph_warmup_iters < 1:
            raise ValueError("cuda_graph_warmup_iters must be >= 1")
