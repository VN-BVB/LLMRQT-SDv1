...
def build_tree_allow_mask_4d(
    ...
    *,
    ...
    ancestors = _ancestor_self_matrix(layout.parent, n, device)
    m = torch.zeros(1, 1, n, n, device=device, dtype=dtype)
    m[0, 0] = ancestors.to(dtype)
    return m
...
def _ancestor_self_matrix(
    ...
    """Return ``[n, n]`` where row i marks node i and all of its ancestors.

    All nodes climb one parent level together, replacing the old Python loop
    over every ``(query, key)`` pair with one indexed tensor write per depth.
    """
    ancestors = torch.eye(n, dtype=torch.bool, device=device)
    if n == 0:
        return ancestors
    ...
    for _ in range(n):
        ...
        if not bool(valid.any()):
            break
        ancestors[idx[valid], cur[valid]] = True
