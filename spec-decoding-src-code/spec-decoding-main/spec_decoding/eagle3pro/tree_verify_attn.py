...
- flash_attn (topk==1): HF flash_attention_2，标准因果attention，不叠树 mask。
- triton_tree (topk>1): Triton extend_attention_fwd + custom_mask。
...
def flash_attn_available() -> bool:
    """是否可用 FlashAttention（需装 flash_attn 且有 CUDA）。"""
    try:
...
def triton_extend_available() -> bool:
    """是否可用 Triton extend attention kernel（需 CUDA + triton + 本包 kernel）。"""
...
def _apply_rotary(
    ...
    """给 q/k 施加 RoPE（用 decoder 传入的 (cos, sin)）；缺失则报错或原样返回。"""
...
def _layer_idx(module: nn.Module) -> int:
    """取该 attention 模块的层号（写 KV cache 时定位层）；缺失则报错。"""
...
def _triton_tree_attn_forward(
    ...
    **kwargs: Any,
    ...
    if head_dim is None:
        q_out = (
            module.qkv_proj.split_sizes[0]
            if hasattr(module, "qkv_proj")
            else module.q_proj.out_features
        )
        head_dim = q_out // num_heads
    if hasattr(module, "qkv_proj"):
        query_states, key_states, value_states = module.qkv_proj(
            hidden_states
        ).split(module.qkv_proj.split_sizes, dim=-1)
        # EAGLE3PRO: target 打包后 Triton 树验证也复用单次 QKV GEMM，避免访问已移除的旧层。
    else:
    ...
    key_states = key_states.view(bsz, q_len, num_kv, head_dim).contiguous()

    # Qwen3 在 RoPE 前还会对每个 attention head 做 Q/K RMSNorm；漏掉会让验证 logits 与原模型不一致。
    if hasattr(module, "q_norm"):
        query_states = module.q_norm(query_states)
    if hasattr(module, "k_norm"):
        key_states = module.k_norm(key_states)
    # EAGLE3PRO: packed split 及 head/seq 转置可能非连续；先按 HF 顺序 norm，再转置给 Triton/vLLM。
    query_states = query_states.transpose(1, 2)
    key_states = key_states.transpose(1, 2)
    value_states = value_states.view(
        bsz, q_len, num_kv, head_dim
    ).transpose(1, 2)
