...
# -----------------------------------------------------------------------------
def _fork_cache_metadata(past_key_values: Any) -> Any:
    """Fork a DynamicCache without copying its immutable prefix tensors.

    ``DynamicLayer.update`` replaces ``keys``/``values`` with ``torch.cat``
    results.  Shallow-copying the cache and every layer therefore lets tree
    verify share the prefix storage while keeping all metadata assignments off
    the canonical decode cache.
    """
    layers = getattr(past_key_values, "layers", None)
    if layers is None:
        raise TypeError("tree verify requires a cache with per-layer metadata")
    forked = copy.copy(past_key_values)
...
def _extend_then_crop(target_m, tree_tokens, past_key_values, *, position_ids, attention_mask):
    """Extend a metadata fork of the prefix cache and discard it after verify.
    ...
    past_key_values
        runner 维护的 canonical prefix KV；本函数不会修改它。
    position_ids
    ...
    """
    verify_cache = _fork_cache_metadata(past_key_values)
    out = target_m(
        tree_tokens,
...
def _select_append_root_child(
    ...
    """
    # Compute every reusable greedy prediction once, then cross the device
    # boundary once.  Shared parents otherwise repeat argmax().item() for each
    # leaf path and serialize the GPU behind Python control flow.
    node_pred = node_logits.argmax(dim=-1)
    root_pred = recovery_logits.argmax(dim=-1).view(1)
    predictions = torch.cat([root_pred, node_pred]).tolist()
    root_pred_int = predictions[0]
    node_pred_list = predictions[1:]
