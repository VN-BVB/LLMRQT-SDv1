...
跑法（Docker 容器内示例；将 <工程根> 换成挂载到容器内的仓库根）::
    ...
    cd <工程根> && PYTHONPATH=. python3 -m spec_decoding.eagle3pro.example.time_stages_eagle3_sgl \\
...
def _load_target(target_id, device, dtype):
    ...
    for mid in [target_id, "NousResearch/Meta-Llama-3.1-8B-Instruct"]:
        try:
            ...
            # transformers 5.x 已弃用 torch_dtype；这里只消除警告，不改变计时精度。
...
def main():
    ...
    import spec_decoding.eagle3pro.runner as R
    ...
    if args.target_optimization != "none":
        ...
        # EAGLE3PRO: 与正式入口相同，target 权重加载后、generator 创建前完成投影打包。
    ...
    if args.target_optimization in (
        ...
        enable_vllm_fused_kernels(
            ...
            in ("packed-vllm-qk", "packed-vllm-flashkv"),
            flash_kv_cache=args.target_optimization == "packed-vllm-flashkv",
        )
        # EAGLE3PRO: profiler 可显式复现正式入口的 target/draft Flash KV 最终路径。
    ...
    # warmup 后把新增的真实 draft 统计也清零，保证报告只覆盖正式区间。
    ...
    import spec_decoding.eagle3pro.tree_verify_full as TVF
    ...
    gen._verify_chain_and_commit = verify_mark(
        "target_verify", gen._verify_chain_and_commit
    )
    # top-k=1 新快路径不再调用 full_tree_verify_*，需单独标记才能正确显示 verify/replay 占比。
    ...
    lines = [
        ...
        f"new_tokens={n_new} rounds={gen.n_rounds} accept_length={accept:.2f} wall={wall:.2f}s spec_tok/s={n_new / wall:.2f}",
        # 该值包含每轮必出的 target token，名称与主 benchmark 的统计口径保持一致。
