...
"""
CPU smoke for EAGLE3: EAGLE3 多层 act 特征 + EAGLE1 静态 top-k 树 + 树掩码 verify。

draft 权重随机（接受率低），但流程可跑通；target verify 决定最终 token。

    PYTHONPATH=. python3 -m spec_decoding.eagle3pro.example.run_smoke_eagle3_sgl --device cpu
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="EAGLE3 tiny smoke")
