...
跑法：
    PYTHONPATH=. python3 -m spec_decoding.eagle3pro.example.run_eagle3_sgl_llama31 \
...
def main(
    *,
    ...
    default_target_optimization: str = "none",
) -> None:
    ...
    p.add_argument(
        ...
        choices=["none", "packed", "packed-vllm-qk", "packed-vllm-flashkv"],
        help=(
            "Qwen3-only inference optimization; packed-vllm-flashkv is the "
            "fastest tested top-1 path and requires the vLLM/FlashAttention CUDA extensions"
        ),
    )
    ...
    if args.target_optimization != "none":
        ...
        # EAGLE3PRO: 权重加载完成后才原地打包；原 checkpoint 和冻结备份均不修改。
    ...
    if args.target_optimization in ("packed-vllm-qk", "packed-vllm-flashkv"):
        ...
        # EAGLE3PRO: draft 权重加载/打包后再挂 vLLM kernel；Flash KV 仅由 top-1 快路径启用。
    print(
        f"[eagle3-e2e] target_optimization={args.target_optimization} "
        f"report={optimization_report}",
        flush=True,
    )
    ...
    rec = (
        ...
        f"target_optimization={args.target_optimization} "
