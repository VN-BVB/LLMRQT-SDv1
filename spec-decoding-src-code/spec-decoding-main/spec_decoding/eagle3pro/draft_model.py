...
class _Eagle3MidLayer(nn.Module):
    ...
    def pack_projections(self) -> None:
        """加载 checkpoint 后合并 QKV 与 gate/up 权重；重复调用安全。"""
        from .target_optim import _PackedLinear

        if not hasattr(self, "qkv_proj"):
            self.qkv_proj = _PackedLinear([self.q_proj, self.k_proj, self.v_proj])
            del self.q_proj
            del self.k_proj
            del self.v_proj
        if not hasattr(self, "gate_up_proj"):
            self.gate_up_proj = _PackedLinear([self.gate_proj, self.up_proj])
            del self.gate_proj
            del self.up_proj
        # EAGLE3PRO: 必须在权重加载完成后转换，避免改变公开 checkpoint 的参数名。

    def _project_qkv(
        self, hidden_states: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if hasattr(self, "qkv_proj"):
            return self.qkv_proj(hidden_states).split(
    ...
    def _mlp(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if hasattr(self, "gate_up_proj"):
            ...
            if getattr(self, "_eagle3pro_vllm_silu", False):
                from .target_optim import _vllm_silu_and_mul
    ...
    def extend(
        ...
        x = torch.cat([e, hn], dim=-1)
        q, k, v = self._project_qkv(x)
        if getattr(past_kv, "_eagle3pro_draft_flash_cache", False):
            q = q.view(b, A, self.n_heads, self.head_dim)
            k = k.view(b, A, self.n_kv_heads, self.head_dim)
            v = v.view(b, A, self.n_kv_heads, self.head_dim)
            o = past_kv.attend_and_update(
                q, k, v, start_position=int(start_position)
            )
            o = o.reshape(b, A, self.n_heads * self.head_dim)
            hidden = residual + self.o_proj(o)
            residual = hidden
            h = self.post_attention_layernorm(hidden)
            # EAGLE3PRO: 最快 top-1 路径由 Flash KV cache 内部完成 RoPE/GQA；数学残差路径不变。
            return residual + self._mlp(h), past_kv
        q = q.view(b, A, self.n_heads, self.head_dim).transpose(1, 2)
        k = k.view(b, A, self.n_kv_heads, self.head_dim).transpose(1, 2)
        v = v.view(b, A, self.n_kv_heads, self.head_dim).transpose(1, 2)
    ...
    def step(
        ...
        k = k.view(b, 1, self.n_kv_heads, self.head_dim).transpose(1, 2)
        v = v.view(b, 1, self.n_kv_heads, self.head_dim).transpose(1, 2)
...
class Eagle3SglDraftModel(nn.Module):
    ...
    def step_flash_cache(
        ...
        """Single-token recursive draft step on the preallocated Flash KV cache."""
        if self.extra_layers:
            raise NotImplementedError("autoregressive step supports num_layers=1 (midlayer only)")
        embeds = self.embed(token_id)
        if cond.dim() == 2:
        ...
        h, cache = self.midlayer.extend(embeds, reduced, cache, int(position))
        logits = self._lm_logits(self.norm(h)[:, -1, :])
        return logits, h[:, -1, :], cache
    ...
    def greedy_target_ids(self, logits: torch.Tensor) -> torch.LongTensor:
        """Top-1 target ids without the unused score/sort output of topk()."""
...
def build_eagle3_sgl_draft(
    ...
    *,
    ...
    if pack_projections:
        m.midlayer.pack_projections()
        # EAGLE3PRO: 先按公开参数名加载 checkpoint，再打包以减少每轮 draft GEMM 数量。
    return m
