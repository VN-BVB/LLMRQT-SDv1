...
def verify_leaf_path_against_parallel_logits(
    ...
    """
    用树 verify 的 [1,L,V] 节点 logits，对一条根→叶路径做 greedy 对齐（旧版规则）。

    Parameters
    ----------
    logits
        整段 tree forward 输出；logits[0, node_idx] 用于核对 token_ids[node_idx]。
    parent, token_ids
        树结构。
    leaf
        叶子节点下标。
    ...
    match_len
        连续接受长度。
    next_token
        全中 = 叶节点 bonus；否则 = 首个不匹配处 target argmax。
    """
